...
def train(opt):
    ...
    image = pre_processing(image[:game_state.screen_width, :int(
        game_state.base_y)], opt.image_size, opt.image_size)
    ...
    while iter < opt.num_iters:
        ...
        if random_action and iter < opt.num_iters / 10:
        ...
        if (iter+1) % 1000000 == 0:
            torch.save(
                model, "{}/flappy_bird_{}".format(opt.saved_path, iter+1))
